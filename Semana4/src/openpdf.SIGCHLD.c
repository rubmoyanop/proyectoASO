...
{
    struct sigaction sa;                            // Se bloquea la señal que se está atendiendo
    memset(&sa, 0, sizeof(struct sigaction));
    sa.sa_handler = signal_handler;
    sa.sa_flags = SA_NOCLDSTOP;        
    sigemptyset(&sa.sa_mask);                       // Mientras se produzca esta señal, podemos bloquear que se produzcan otras señasles
...
{
    int saved_errno = errno;    // Guardamos el errno para luego devolverlo, una señal no debe modificar el errno
    int estado2; // Para el último ejercicio
