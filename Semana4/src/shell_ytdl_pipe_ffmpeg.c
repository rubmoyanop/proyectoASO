...
int main(int argc, char *argv[])
{
    ...
    case 0: /* Hijo izquierdo de la tubería */
        ...
        execlp("./youtube-dl", "youtube-dl", url, "-q", "-o", "-", NULL); // NULL indica que ya no hay más argumentos
    ...
    case 0: /* Hijo derecho de la tubería  */
        int fderr; // ejercicio
        if(-1 == (fderr= open("ffmpeg.log", O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR))){
            perror("open(0)");
            exit(EXIT_FAILURE);
        }

        if(-1== (dup2(fderr, STDERR_FILENO))){ // Copiamos el descriptor fderr en la salida estándar de error (primero cierra STDERR)
            perror("dup2(1)");
            exit(EXIT_FAILURE);
        }

        if( -1== (close(fderr))){ // Ya puedo dejar de usar el descriptor de fichero del archivo del log
            error("close(3)");
            exit(EXIT_FAILURE);
        } 
